...
**Это уже сделано, не только план.** Сайт и админка развёрнуты и работают — текущее
состояние (IP сервера, что с DNS, что ещё не доделано) смотри в
[CLAUDE.md → «Статус запуска на хостинг»](../CLAUDE.md#статус-запуска-на-хостинг).
Инструкция ниже пригодится, если разворачиваете заново или поднимаете второй сервер.
...
**3.1. Почта для заказов**

Форма на `/cart/` шлёт заказы письмом через `admin/mail.mjs` — ему нужен
`content/.smtp.json` (в `.gitignore`, в репозитории его нет, заводится на сервере руками):

```bash
sudo -u axelnn tee /var/www/axelnn/content/.smtp.json <<'EOF'
{
 "host": "smtp.yandex.ru",
 "port": 465,
 "secure": true,
 "user": "sales-axelnn@axelnn.ru",
 "pass": "пароль приложения, не обычный пароль от почты",
 "to": "sales-axelnn@axelnn.ru"
}
EOF
```

Без файла `/api/order` отвечает `503`, а форма на сайте просит позвонить — сайт
при этом не ломается, просто заказы идут только по телефону.
