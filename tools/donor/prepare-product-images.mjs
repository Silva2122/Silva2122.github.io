...
// где полторы сотни карточек, нельзя. Приводим к 1200 и жмём в webp q90 —
// заметно выше того, что вообще видно на экране (см. admin/images.mjs
// про числа 600/1000 → 900/1800 → 1200/2400 и почему выше уже смысла нет).
//
...
// Галерея страницы товара: кадр там показывается на ~600-770 логических
// пикселей в зависимости от ширины экрана, на плотных экранах это уже за
// 2300 — 2400 берём как потолок с небольшим запасом, withoutEnlargement
// не даст растянуть то, что мельче (а оригиналы донора часто и не дотягивают).
const GSIZE = Number(arg('--gallery-size', 2400));
