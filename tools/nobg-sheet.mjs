...
//
//   node tools/nobg-sheet.mjs .shots/nobg/birefnet
import sharp from 'sharp';
import { readdirSync, readFileSync, writeFileSync } from 'node:fs';
import { join, basename } from 'node:path';

const DIR = process.argv[2] || '.shots/nobg/birefnet';
...
// В папке могут лежать кадры прошлых прогонов — ориентируемся на отчёт, а не на readdir.
const all = readdirSync(DIR).filter((f) => /\.(png|webp)$/.test(f) && byFile.has(f)).sort();

// Из трёх сотен кадров лист не собрать: берём равномерно по всему списку,
// иначе выборка окажется из одного раздела и о качестве ничего не скажет.
const N = Number(process.argv[4] || 24);
const step = Math.max(1, Math.floor(all.length / N));
const files = all.length > N ? Array.from({ length: N }, (_, i) => all[i * step]).filter(Boolean) : all;
console.log(`Кадров в отчёте: ${all.length}, беру ${files.length}`);
...
// Кладём рядом с исходниками, а не в корень репозитория: .shots/ уже в .gitignore.
const out = join(DIR, `_sheet-${basename(DIR)}.png`);
