...
function collect() {
  ...
  // Второй аргумент обязателен. Google Fonts режет гарнитуру на поднаборы
  // с разными unicode-range, и браузер тянет только те, чьи символы есть
  // на странице. Без текста check() проверяет латиницу — на странице,
  // где Playfair стоит в одном кириллическом заголовке, латинский поднабор
  // не загружается, и проверка врала «отрисован фолбэк» на исправной странице.
  for (const f of ['Montserrat', 'Playfair Display']) {
    R.fonts[f] = document.fonts.check(`16px "${f}"`, 'Аа') || document.fonts.check(`16px "${f}"`, 'Aa');
  }
